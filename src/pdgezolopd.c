               ...
               int *info, int zolo_r)
{
    ...
    *info = 0;
     ...
     */
    if (verbose && myrank_mpi == 0)
        fprintf(stderr, "Testing the input parameters \n");
    if (nprow_all == -1)
    {
        *info = -(700 + ctxt_);
    }
    else
    {
        ...
        if (verbose && myrank_mpi == 0)
            fprintf(stderr, "Testing the input parameters 280  \n");
        ...
        if (*info == 0)
        {
            ...
            if (optcond)
            {
                pdgeqrf_(&nb, &M, A_all, &i1, &i1, descA_all, tau_all, Work2, &lmin2, &iinfo);
            }
            else
            {
                pdgecon_("1", &M, A_all, &i1, &i1, descA_all, &Anorm, &Li, Work2, &lmin2, Wi_all, &lWi_all, &iinfo);
            }
            lmin2 = (int)Work2[0];
            lWi_all = Wi_all[0];
            // Work1[0] = lmin1;
        ...
        idum2[0] = 1;
        idum2[1] = 15;
        pchk1mat_(&M, &i2, &N, &i3, &iA_all, &jA_all, descA_all, &i7, &i2, idum1, idum2, info);
        // if ((*info == 0) && wantH){
        if ((*info == 0))
        {
            pchk1mat_(&M, &i2, &N, &i3, &iB_all, &jB_all, descB_all, &i11, &i0, idum1, idum2, info);
        }
    }
    ...
    else if (lquery)
    {
        ...
        if (optcond)
        {
            pdgeqrf_(&nb, &M, A_all, &i1, &i1, descA_all, tau_all, Work2, &lWork2, &iinfo);
        }
        else
        {
        ...
        return 0;
        if (verbose && myrank_mpi == 0)
            fprintf(stderr, "Testing the input parameters 368  \n");
    }
    ...
    */
     ...
     */
    pdlacpy_("All", &M, &N, A_all, &i1, &i1, descA_all, Acpy_all, &i1, &i1, descAcpy_all);
    ...
    if (verbose && myrank_mpi == 0)
    {
        fprintf(stderr, "Cond estimate starts\n");
    }
    if (prof)
    {
        litime = -MPI_Wtime();
    ...
    if (!optcond)
    {
        pdgecon_("1", &M, A_all, &i1, &i1, descA_all, &Anorm, &Li, Work2, &lWork21, Wi_all, &lWi_all, &iinfo);
        lWork21 = (int)Work2[0];
        lWi_all = Wi_all[0];
    ...
    if (optcond)
    {
        ...
        pdgeqrf_(&M, &N, A_all, &i1, &i1, descA_all, tau_all, Work2, &lWork2, &iinfo);
        pdtrtri_("U", "N", &N, A_all, &i1, &i1, descA_all, &iinfo);
        double Ainvnorm = pdlange_("1", &M, &N, A_all, &i1, &i1, descA_all, Work2);
        Li = Ainvnorm * Anorm;
        Li = Anorm / Li;
        // Li = Li/sqrt(N); // This reduces the accuracy
        *flops += FLOPS_DGEQRF(M, N) + FLOPS_DTRTRI(N);
    }
    else
    {
    ...
    if (verbose && myrank_mpi == 0)
    {
        fprintf(stderr, " Computing the second norm estimate\n");
    }
    if (prof)
    {
        nrmtime = -MPI_Wtime();
    ...
    pdlacpy_("All", &M, &N, Acpy_all, &i1, &i1, descAcpy_all, A_all, &i1, &i1, descA_all);
    pdlascl_("G", &norm_est, &alpha, &M, &N, A_all, &i1, &i1, descA_all, &iinfo);
    ...
    if (verbose && myrank_mpi == 0)
    {
        fprintf(stderr, " Computing the number of processor per subproblem\n");
    }
    ...
    if (myrank_mpi == 0)
    {
        ...
        {
            fprintf(stderr,
                    " The number of processors per subproblem is %d, for a better grid configuration we will use %d "
                    "with grid configuration %d x %d\n",
    ...
    */
     ...
     */
    if (verbose && myrank_mpi == 0)
        fprintf(stderr, " Map processes to different context\n");
     ...
     */
    if (verbose && myrank_mpi == 0)
        fprintf(stderr, "Desc Init and ctxt create of subproblems\n");
    ...
    if (myrank_mpi < max_rank)
    {
        ...
        *(ictxt_id + (int)(myrank_mpi / (nprow * npcol))) = ictxt;
         ...
         */
        if (verbose && myrank_mpi == 0)
            fprintf(stderr, "Start copy the input matrix to U in the different ctxt \n");
        // descU[1] = ictxt;
        ...
        for (j = 0; j < nbprob; j++)
        {
            ...
            pdgemr2d_(&M, &N, A_all, &i1, &i1, descA_all, U, &i1, &i1, descU, &ictxt_all);
            if (group_id != j)
            {
                descU[1] = temp;
            }
        ...
        */
         ...
         */
        if (verbose && myrank_mpi == 0)
            fprintf(stderr, " Allocate U_ac on group0\n");
        ...
        while (myrank_mpi < max_rank && it < itmax)
        {
             ...
             */
            pdlacpy_("A", &M, &N, U, &i1, &i1, descU, Ucpy, &i1, &i1, descUcpy);
            ...
            if (it == 1)
            {
                if (verbose && myrank_mpi == 0)
                {
                    fprintf(stderr, "Start the ZOLOPD iterations\n");
                }
                // qrtime = 0.0;
                if (prof)
                {
                ...
                if (howqr)
                {
                    ...
                    if (verbose && myrank_mpi == 0)
                    {
                        fprintf(stderr, "Start the QR based first iteration\n");
                    }
                    pdlacpy_("A", &M, &N, Ucpy, &i1, &i1, descUcpy, B, &i1, &i1, descB);
                    alpha = 0.;
                    beta = sqrt(c[2 * ii - 2]);
                     ...
                     */
                    pdgeqrf_(&MB, &N, B, &i1, &i1, descB, tau, Work, &lWork3, &iinfo);
                    pdorgqr_(&MB, &N, &N, B, &i1, &i1, descB, tau, Work, &lWork3, &iinfo);
                     ...
                     */
                    alpha = -1. * enu / den / sqrt(c[2 * ii - 2]);
                    beta = 0.;
                    if (ii == 1)
                    {
                        beta = 1.;
                    }
                    pdgemm_("N", "T", &M, &N, &N, &alpha, B, &i1, &i1, descB, B, &iM, &i1, descB, &beta, U, &i1, &i1,
                            descU);
                ...
                else
                {
                    ...
                    if (verbose && myrank_mpi == 0)
                    {
                        fprintf(stderr, "Start the Chol based first iteration\n");
                    }
                     ...
                     */
                    alpha = 0.;
                    pdlaset_("G", &M, &N, &alpha, &alpha, B, &i1, &i1, descB);
                    // pdlacpy_( "U", &M, &N, C, &i1, &i1, descC, B, &i1, &i1, descB );
                    Rnorm = pdlange_("1", &M, &N, B, &i1, &i1, descB, Work);
                    pdtrtri_("U", "N", &N, B, &i1, &i1, descB, &iinfo);
                    Rinvnorm = pdlange_("1", &M, &N, B, &i1, &i1, descB, Work);
            ...
            {   // Chol-based second iteration
                /**
                 * Compute C = U * U' + c[2*ii-1] * I
                 */
                if (verbose && myrank_mpi == 0)
                {
                    fprintf(stderr, "Start the Chol based second iteration\n");
                }
                // potime = 0.0;
                if (prof)
                {
                ...
                alpha = 0.;
                beta = 1.;
                pdlaset_("G", &M, &N, &alpha, &beta, B, &i1, &i1, descB);
                alpha = 1.;
                beta = c[2 * ii - 2];
                pdgemm_("T", "N", &M, &N, &N, &alpha, Ucpy, &i1, &i1, descUcpy, Ucpy, &i1, &i1, descUcpy, &beta, B, &i1,
                        &i1, descB);
                /**
                 * dpotrf on R
                 */
                pdpotrf_("U", &M, B, &i1, &i1, descB, &iinfo);
                ...
                alpha = 1.;
                beta = 0.;
                pdlacpy_("A", &M, &N, Ucpy, &i1, &i1, descUcpy, A, &i1, &i1, descA);
                pdtrsm_("R", "U", "N", "N", &M, &N, &alpha, B, &i1, &i1, descB, A, &i1, &i1, descA);
                /*
                 * solve Qtmp=Qtmp/Cinv';
                 */
                pdtrsm_("R", "U", "T", "N", &M, &N, &alpha, B, &i1, &i1, descB, A, &i1, &i1, descA);
                alpha = -enu / den;
                beta = 0.;
                if (ii == 1)
                {
                ...
                pdgeadd_("N", &M, &N, &alpha, A, &i1, &i1, descA, &beta, U, &i1, &i1, descU);
                /* Main flops used in this step */
                flops_dgemm = FLOPS_DGEMM(M, N, N);
                flops_dpotrf = FLOPS_DPOTRF(M);
                flops_dtrsm = 2. * FLOPS_DTRSM('R', M, N);
            ...
            if (prof)
            {
                time_gather = -MPI_Wtime();
            }
            if (verbose && myrank_mpi == 0)
            {
                fprintf(stderr, "Gather the result matrix from different ctxt\n");
            }
            alpha = 1.;
            int j;
            ...
            if (it < itmax)
            {
                ...
                for (j = 1; j < nbprob; j++)
                {
                    ...
                    MPI_Barrier(new_comm);
                    pdgemr2d_(&M, &N, U, &i1, &i1, descU, U_ac, &i1, &i1, descU_ac, &ictxt_all);
                    MPI_Barrier(new_comm);
                    ...
                    {
                        descU[1] = ictxt;
                        pdgeadd_("N", &M, &N, &alpha, U_ac, &i1, &i1, descU_ac, &alpha, U, &i1, &i1, descU);
                        descU[1] = -1;
                    }
                }
                ...
                if (group_id == 0)
                {
                     ...
                     * U = U/ff(1);
                     */
                    pdlascl_("G", &ff_1, &alpha, &M, &N, U, &i1, &i1, descU, &iinfo);
                    /**
                     * if symm;    U=(U'+U)/2;end    // force symmetry if A symmetric
                     */
                    if (symm)
                    {
                        ...
                        alpha = 0.5;
                        pdgeadd_("T", &M, &N, &alpha, B, &i1, &i1, descB, &alpha, U, &i1, &i1, descU);
                    }
                }
            }
            else
            {
                ...
                for (j = 0; j < nbprob; j++)
                {
                    ...
                    MPI_Barrier(new_comm);
                    pdgemr2d_(&M, &N, U, &i1, &i1, descU, B_all, &i1, &i1, descB_all, &ictxt_all);
                    beta = 1.;
                    if (j == 0)
                    {
                        beta = 0.;
                    }
                    pdgeadd_("N", &M, &N, &alpha, B_all, &i1, &i1, descB_all, &beta, A_all, &i1, &i1, descA_all);
                ...
                if (symm)
                {
                    ...
                    alpha = 0.5;
                    pdgeadd_("T", &M, &N, &alpha, B_all, &i1, &i1, descB_all, &alpha, A_all, &i1, &i1, descA_all);
                }
            }
            MPI_Barrier(new_comm);
            if (prof)
            ...
            if (verbose && myrank_mpi == 0)
            {
                fprintf(stderr, "Bcast the result matrix to all the ctxt's\n");
            }
            ...
            if (it < itmax)
            {
                ...
                if (group_id == 0)
                {
                    pdlacpy_("A", &M, &N, U, &i1, &i1, descU, U_ac, &i1, &i1, descU_ac);
                    descU[1] = -1;
                }
                ...
                for (j = 1; j < nbprob; j++)
                {
                    ...
                    MPI_Barrier(new_comm);
                    pdgemr2d_(&M, &N, U_ac, &i1, &i1, descU_ac, U, &i1, &i1, descU, &ictxt_all);
                    MPI_Barrier(new_comm);
                    if (group_id != j)
                    {
                        descU[1] = ictxt;
        ...
        if (ns)
        {
            ...
            if (verbose && myrank_mpi == 0)
            {
                fprintf(stderr, "# \tNewton-Schulz refinement \n");
            }
            // nstime = 0.0;
            if (prof)
            {
            ...
            alpha = 1. / 2.;
            beta = 0.;
            pdgemm_("T", "N", &M, &N, &N, &alpha, A_all, &i1, &i1, descA_all, A_all, &i1, &i1, descA_all, &beta, B_all,
                    &i1, &i1, descB_all);
            alpha = -1.;
            beta = 3. / 2.;
            ...
            pdgemr2d_(&M, &N, Acpy_all, &i1, &i1, descAcpy_all, B, &i1, &i1, descB, &ictxt_all);
            if (group_id != j)
            {
                descB[1] = ictxt;
            }
            pdlacpy_("A", &M, &N, A_all, &i1, &i1, descA_all, Acpy_all, &i1, &i1, descAcpy_all);
            pdgemm_("N", "N", &M, &N, &N, &alpha, Acpy_all, &i1, &i1, descAcpy_all, B_all, &i1, &i1, descB_all, &beta,
                    A_all, &i1, &i1, descA_all);
            ...
            pdgemr2d_(&M, &N, B, &i1, &i1, descB, Acpy_all, &i1, &i1, descAcpy_all, &ictxt_all);
            if (group_id != j)
            {
                descB[1] = ictxt;
            }
         ...
         */
        if (verbose && myrank_mpi == 0)
            fprintf(stderr, "Computing the symmetric positive semidefinite matrix H \n");
        if (wantH)
        {
            if (prof)
            {
            ...
            alpha = 1.0;
            beta = 0.0;
            pdgemm_("T", "N", &M, &N, &N, &alpha, A_all, &i1, &i1, descA_all, Acpy_all, &i1, &i1, descAcpy_all, &beta,
                    B_all, &i1, &i1, descB_all);
            pdlacpy_("A", &M, &N, B_all, &i1, &i1, descB_all, Acpy_all, &i1, &i1, descAcpy_all);
            alpha = 0.5;
            pdgeadd_("T", &M, &N, &alpha, Acpy_all, &i1, &i1, descAcpy_all, &alpha, B_all, &i1, &i1, descB_all);
        ...
        if (prof && myrank_mpi == 0)
        {
            ...
            fprintf(stderr, "#\n");
            fprintf(stderr, "# \tN    \ttimeZOLOPD     \ttimeLi   \ttime2nr     #itmax  \ttime1itQR   \t#QR  "
                            "\treduced_time_gather  \treduced_time_bcast  \ttime1itPO   \t#PO    \ttimeFormH \n");
            fprintf(stderr,
                    "  \t%d \t%2.4e \t%2.4e \t%2.4e  \t%d  \t%2.4e  \t%d \t%2.4e  \t\t%2.4e  \t\t%2.4e \t%d \t%2.4e \n",
                    M, reduced_qwtime, reduced_litime, reduced_nrmtime, itmax, reduced_qrtime, itqr * nbprob,
                    reduced_time_gather, reduced_time_bcast, reduced_potime, itpo * nbprob, reduced_Htime);
        }
        if (myrank_mpi == 0)
        {
            fprintf(stderr, "#\n");
    ...
    else
    {
        ...
        while (it1 < itmax)
        {
            ...
            MPI_Status stat;
            MPI_Recv(&ff_1, 1, MPI_DOUBLE, myrank_mpi - max_rank, 0, MPI_COMM_WORLD, &stat);
