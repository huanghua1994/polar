...
int pdgeqdwh(char *jobh, int m, int n, double *A, int iA, int jA, int *descA, double *H, int iH, int jH, int *descH,
             double *Work1, int lWork1, double *Work2, int lWork2, int *info)
{
    ...
    *info = 0;
    ...
    else
    {
        ...
        idum2[0] = 1;
        idum2[1] = 15;
        pchk1mat_(&m, &i2, &n, &i3, &iA, &jA, descA, &i7, &i2, idum1, idum2, info);
        if ((*info == 0) && wantH)
        {
            pchk1mat_(&m, &i2, &n, &i3, &iH, &jH, descH, &i11, &i0, idum1, idum2, info);
        }
    }
    ...
    {
        fprintf(stderr, "Entering QDWH\n");
    }
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "Preparing workspace for QDWH\n");
    }
    qwtime = 0.0;
    if (prof)
    {
    ...
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "Finish preparing workspace for QDWH\n");
    }
    ...
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "lacpy ends\n");
    }
    Anorm = pdlange_("1", &m, &n, U, &i1, &i1, descU, H);
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "dlange ends\n");
    }
    ...
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "Cond estimate ends\n");
    }
    ...
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "QDWH loop starts\n");
    }
    if (myrank_mpi == 0)
    {
        fprintf(stderr, "\nItConv %d itcqr %d itcpo %d norm_est %2.4e Li %2.4e \n", itconv, itcqr, itcpo, norm_est, Li);
        fprintf(stderr, "It Facto Conv\n");
    }
    it = 0;
    ...
    while (it < itconv)
    {
        ...
        if (c > 100)
        {
            ...
            alpha = (a - b / c) / sqrt(c);
            beta = (b / c);
            pdgemm_("N", "T", &m, &n, &n, &alpha, B, &i1, &i1, descB, B, &iM, &i1, descB, &beta, A, &i1, &i1, descA);
        ...
        else
        {
            ...
            pdgemm_("T", "N", &m, &n, &n, &c, A, &i1, &i1, descA, A, &i1, &i1, descA, &beta, H, &i1, &i1, descH);
        ...
        if (verbose && (myrank_mpi == 0))
            fprintf(stderr, "%02d %-5s %e\n", it, facto == 0 ? "QR" : "PO", conv);
    }
    if (verbose && (myrank_mpi == 0))
    {
        fprintf(stderr, "QDWH loop ends\n");
    }
    ...
    alpha = 1.0;
    beta = 0.0;
    pdgemm_("T", "N", &m, &n, &n, &alpha, A, &i1, &i1, descA, U, &i1, &i1, descU, &beta, H, &i1, &i1, descH);
